"""
Fixed DINOv3 Patch Matching with ATTENTION-BASED RANKING (v79 + edge suppression + grid bounds fix)
...
- Edge suppression: Ignore edge patches via --border-patches, mask erosion via --mask-erode-px,
  and optional attention-intersection foreground via --attn-fg
- CRITICAL FIX v79: Constrain keypoints to foreground mask and limit grid expansion to prevent
  out-of-bounds errors after TPS warping, especially with mask erosion
"""
...
def expand_crop_bbox_for_keypoints(
    crop_bbox: Optional[Tuple[int, int, int, int]],
    keypoints: np.ndarray,
    img_original_size: Tuple[int, int],
    margin: float = 0.05
) -> Tuple[int, int, int, int]:
    """
    Expand the crop bounding box to ensure all keypoints are included.
    
    Args:
        crop_bbox: Current crop bbox (x0, y0, x1, y1) or None
        keypoints: Keypoint coordinates in original image space (N x 2)
        img_original_size: (width, height) of original image
        margin: Additional margin as fraction of bbox size
        
    Returns:
        Expanded crop bbox (x0, y0, x1, y1) that includes all keypoints
    """
    if crop_bbox is None or keypoints is None or len(keypoints) == 0:
        return crop_bbox
    
    x0, y0, x1, y1 = crop_bbox
    orig_w, orig_h = img_original_size
    
    # Get keypoint bounds
    kpt_x_min, kpt_x_max = keypoints[:, 0].min(), keypoints[:, 0].max()
    kpt_y_min, kpt_y_max = keypoints[:, 1].min(), keypoints[:, 1].max()
    
    # Check if keypoints are within current crop
    if kpt_x_min >= x0 and kpt_x_max < x1 and kpt_y_min >= y0 and kpt_y_max < y1:
        # All keypoints already within crop
        return crop_bbox
    
    # Expand to include keypoints
    new_x0 = min(x0, kpt_x_min)
    new_y0 = min(y0, kpt_y_min)
    new_x1 = max(x1, kpt_x_max)
    new_y1 = max(y1, kpt_y_max)
    ...
    new_x0 = max(0, new_x0 - pad_x)
    new_y0 = max(0, new_y0 - pad_y)
    new_x1 = min(orig_w, new_x1 + pad_x)
    new_y1 = min(orig_h, new_y1 + pad_y)
    
    # Warn if we had to expand
    if (new_x0 < x0 or new_y0 < y0 or new_x1 > x1 or new_y1 > y1):
        print(f"⚠️  WARNING: Expanded crop bbox to include keypoints")
        print(f"    Original bbox: ({x0}, {y0}, {x1}, {y1})")
        print(f"    Expanded bbox: ({new_x0}, {new_y0}, {new_x1}, {new_y1})")
        print(f"    Keypoint range: x=[{kpt_x_min:.0f}, {kpt_x_max:.0f}], y=[{kpt_y_min:.0f}, {kpt_y_max:.0f}]")
    
    return (int(new_x0), int(new_y0), int(new_x1), int(new_y1))

# ========================= NEW: Keypoint reconciliation helpers (v81) =========================
def _load_coco_keypoints_indexed(coco_json_path: str, image_path: str):
    """
    Return keypoints for an image preserving COCO index order.
    Returns (kpts_xyv, names) where kpts_xyv is (N,3) with NaNs for missing indices.
    Does not drop any positions; missing entries are set to NaN.
    """
    import numpy as _np
    from pathlib import Path as _Path
    import json as _json

    with open(coco_json_path, 'r') as _f:
        _coco = _json.load(_f)

    # N expected from categories[0].keypoints (if present)
    _N = None
    _names = None
    cats = _coco.get("categories", [])
    if cats and isinstance(cats[0], dict) and "keypoints" in cats[0]:
        _names = cats[0]["keypoints"]
        _N = len(_names)

    # file_name -> image_id
    fname2id = {str(img["file_name"]): str(img["id"]) for img in _coco.get("images", [])}

    filename = _Path(image_path).name
    image_id = fname2id.get(filename)
    if image_id is None:
        # fallback to stem-only matching if file extensions differ
        stem = _Path(image_path).stem
        for fn, iid in fname2id.items():
            if _Path(fn).stem == stem:
                image_id = iid
                break
    ...
    if image_id is None:
        ...
        if _N is not None:
            return _np.full((_N, 3), _np.nan, dtype=float), _names
        return None, _names

    anns = [a for a in _coco.get("annotations", []) if str(a.get("image_id")) == str(image_id)]
    if not anns:
        if _N is not None:
            return _np.full((_N, 3), _np.nan, dtype=float), _names
        return None, _names

    # take first annotation (as you do now)
    kpts = _np.array(anns[0]["keypoints"], dtype=float).reshape(-1, 3)

    # If category didn't tell us N, use the length found
    if _N is None:
        _N = kpts.shape[0]

    # Ensure fixed length N with NaNs for missing indices
    out = _np.full((_N, 3), _np.nan, dtype=float)
    m = min(_N, kpts.shape[0])
    out[:m] = kpts[:m]
    return out, _names
...
def _fit_similarity(A, B):
    """
    Estimate similarity transform mapping B -> A using Procrustes-like method.
    Needs at least 2 common points.
    Returns (s, R, t) so that A ≈ s * (B @ R) + t.
    """
    import numpy as _np
    ...
    normA = _np.linalg.norm(Ac) + 1e-8
    normB = _np.linalg.norm(Bc) + 1e-8
    Ac /= normA
    Bc /= normB
    H = Bc.T @ Ac
    ...
    if _np.linalg.det(R) < 0:
        ...
        R = U @ Vt
    s = (normA / normB)
    t = A.mean(axis=0) - (B.mean(axis=0) @ R) * s
    return s, R, t


def _apply_similarity(X, s, R, t):
    return (X @ R) * s + t


def reconcile_keypoints_by_index(coco_json_path: str, imgA_path: str, imgB_path: str):
    """
    Load COCO KPs for both images preserving index order and impute any missing index on either side.

    Steps:
      1) Load (N,3) arrays with NaNs for missing slots (preserving COCO index order).
      2) Use indices present on BOTH to estimate a similarity transform.
      3) If an index is missing on A but present on B, map B[j] -> A-space (fill).
         If missing on B but present on A, map A[j] -> B-space (fill).
      4) If <2 common points, fill missing with the centroid of available landmarks.

    Returns:
      kA_xy (N,2), kB_xy (N,2)  (no NaNs after imputation)
    """
    import numpy as _np

    kA_xyv, _ = _load_coco_keypoints_indexed(coco_json_path, imgA_path)
    kB_xyv, _ = _load_coco_keypoints_indexed(coco_json_path, imgB_path)
    if kA_xyv is None or kB_xyv is None:
        return None, None

    kA = kA_xyv[:, :2].astype(float)
    kB = kB_xyv[:, :2].astype(float)
    maskA = ~_np.isnan(kA).any(axis=1)
    maskB = ~_np.isnan(kB).any(axis=1)
    common = maskA & maskB

    if common.sum() >= 2:
        sAB, RAB, tAB = _fit_similarity(kA[common], kB[common])  # map B -> A
        sBA, RBA, tBA = _fit_similarity(kB[common], kA[common])  # map A -> B
    else:
        sAB = RAB = tAB = None
        sBA = RBA = tBA = None

    # fill missing on A using B
    missingA = (~maskA) & maskB
    if missingA.any():
        if sAB is not None:
            kA[missingA] = _apply_similarity(kB[missingA], sAB, RAB, tAB)
        else:
            centroid = kA[maskA].mean(axis=0) if maskA.any() else _np.array([0.0, 0.0])
            kA[missingA] = centroid

    # fill missing on B using A
    missingB = (~maskB) & maskA
    if missingB.any():
        if sBA is not None:
            kB[missingB] = _apply_similarity(kA[missingB], sBA, RBA, tBA)
        else:
            centroid = kB[maskB].mean(axis=0) if maskB.any() else _np.array([0.0, 0.0])
            kB[missingB] = centroid

    # both missing (rare): place at centroid to keep arrays numeric
    both_missing = (~maskA) & (~maskB)
    if both_missing.any():
        kA[both_missing] = _np.nan_to_num(kA.mean(axis=0), nan=0.0)
        kB[both_missing] = _np.nan_to_num(kB.mean(axis=0), nan=0.0)

    return kA, kB
# =============================================================================================


def clip_keypoints_to_canvas(
    keypoints: np.ndarray,
    img_size: int,
    identifier: str = "image"
) -> Tuple[np.ndarray, bool]:
    """
    Clip keypoints to valid image canvas bounds and warn if any were outside.
    
    Args:
        keypoints: Keypoint coordinates (N x 2)
        img_size: Size of the square canvas
        identifier: Image identifier for warning messages
        
    Returns:
        (clipped_keypoints, had_out_of_bounds)
    """
    if keypoints is None or len(keypoints) == 0:
        return keypoints, False
    
    original_kpts = keypoints.copy()
    had_out_of_bounds = False
    
    # Check for out-of-bounds keypoints
    out_of_bounds_mask = (
        (keypoints[:, 0] < 0) | (keypoints[:, 0] >= img_size) |
        (keypoints[:, 1] < 0) | (keypoints[:, 1] >= img_size)
    )
    
    if np.any(out_of_bounds_mask):
        had_out_of_bounds = True
        n_out = out_of_bounds_mask.sum()
        
        print(f"⚠️  WARNING: {n_out}/{len(keypoints)} keypoints fall outside canvas bounds for {identifier}")
        print(f"    Canvas size: [0, {img_size}) x [0, {img_size})")
        
        # Show which keypoints are problematic
        for i, is_out in enumerate(out_of_bounds_mask):
            if is_out:
                x, y = keypoints[i]
                print(f"    Keypoint {i}: ({x:.1f}, {y:.1f}) -> ", end="")
                # Clip to valid range
                new_x = np.clip(x, 0, img_size - 1)
                new_y = np.clip(y, 0, img_size - 1)
                print(f"clipped to ({new_x:.1f}, {new_y:.1f})")
                keypoints[i] = [new_x, new_y]
    
    return keypoints, had_out_of_bounds


def constrain_keypoints_to_mask(
    keypoints: np.ndarray,
    mask: np.ndarray,
    img_size: int,
    safety_margin: int = 5,
    ...
    """
    Constrain keypoints to stay within the foreground mask with a safety margin.
    If a keypoint is outside the mask, move it to the nearest valid point inside the mask.
    
    This prevents TPS warping issues where keypoints near mask edges or outside eroded
    masks cause the warped grid to extend beyond valid image bounds.
    
    Args:
        keypoints: [N, 2] keypoint coordinates
        mask: [H, W] boolean mask of foreground
        img_size: Size of image
        safety_margin: Additional pixels to stay away from mask edge
        identifier: Image identifier for logging
        
    Returns:
        (constrained_keypoints, had_corrections)
    """
    if keypoints is None or len(keypoints) == 0:
        return keypoints, False
    ...
    # Erode mask slightly to create safety margin
    if safety_margin > 0:
    ...
    valid_coords = np.argwhere(safe_mask)  # [N_valid, 2] in (y, x) format
    
    if len(valid_coords) == 0:
        print(f"⚠️  ERROR: No valid foreground pixels in {identifier} after safety margin!")
        return kpts, True
    
    # Convert to (x, y) format for distance calculation
    valid_coords_xy = valid_coords[:, [1, 0]]  # Swap to (x, y)
    
    for i, kpt in enumerate(kpts):
        x, y = kpt[0], kpt[1]
        
        # Check if point is outside safe region
        ix, iy = int(round(x)), int(round(y))
        if not (0 <= ix < img_size and 0 <= iy < img_size and safe_mask[iy, ix]):
            had_corrections = True
            
            # Find nearest valid point
            distances = np.sqrt((valid_coords_xy[:, 0] - x)**2 + (valid_coords_xy[:, 1] - y)**2)
            nearest_idx = distances.argmin()
            new_x, new_y = valid_coords_xy[nearest_idx]
            
            print(f"    Keypoint {i} for {identifier}: ({x:.1f}, {y:.1f}) → ({new_x:.1f}, {new_y:.1f})")
            kpts[i] = [new_x, new_y]
    
    if had_corrections:
        print(f"⚠️  Constrained keypoints for {identifier} to valid foreground region")
    
    return kpts, had_corrections
...
def create_consensus_grid(consensus_keypoints: np.ndarray,
                          ...
                          margin: float = 0.05,  # REDUCED from 0.1 to prevent grid going OOB
    ...
    CRITICAL FIX v79: Use smaller margin (0.05 instead of 0.1) to reduce chance of
    warped grid points falling outside valid image bounds after TPS transformation.
    """
...
def warp_grid_to_specimen(grid_info: Dict, 
                         tps_warper: TPSGridWarper,
                         img_size: int = 512,
                         safety_margin: int = 10) -> Dict:
    """
    Warp grid from consensus space to specimen space.
    
    CRITICAL FIX v79: Constrain warped grid points to stay within valid image bounds
    with a safety margin. This prevents grid cells from extending outside the image
    after TPS transformation, which causes IndexError when mapping patches to cells.
    
    Args:
        grid_info: Grid definition in consensus space
        tps_warper: TPS warper (consensus to specimen)
        img_size: Size of the image canvas (default 512)
        safety_margin: Pixels to stay away from edges (default 10)
    ...
    # CRITICAL FIX v79: Constrain warped points to valid bounds with safety margin
    # This prevents the warped grid from extending outside the image, which causes
    # issues when keypoints are near mask edges or after mask erosion
    min_coord = safety_margin
    max_coord = img_size - safety_margin - 1
    
    # Count how many points needed clipping
    n_clipped = np.sum((warped_intersections[:, 0] < min_coord) | 
                       (warped_intersections[:, 0] > max_coord) |
                       (warped_intersections[:, 1] < min_coord) | 
                       (warped_intersections[:, 1] > max_coord))
    
    if n_clipped > 0:
        print(f"    Clipped {n_clipped}/{len(warped_intersections)} warped grid points to valid bounds")
    
    warped_intersections[:, 0] = np.clip(warped_intersections[:, 0], min_coord, max_coord)
    warped_intersections[:, 1] = np.clip(warped_intersections[:, 1], min_coord, max_coord)
...
def tps_homology_pipeline(
    ...
    if tps_rows is not None:
        ...
        grid_info = create_consensus_grid(consensus_shape, rows=tps_rows, cols=tps_cols, margin=0.05)
    else:
        print(f"Creating {chosen_cell_size}Ã—{chosen_cell_size} px grid in consensus space...")
        grid_info = create_consensus_grid(consensus_shape, cell_size=chosen_cell_size, margin=0.05)
    ...
    warped_grid_A = warp_grid_to_specimen(grid_info, warperA, img_size=img_size)
    warped_grid_B = warp_grid_to_specimen(grid_info, warperB, img_size=img_size)
...
#            aligner = KeypointAligner(args.keypoints_json)
#            
#            kptsA_orig = aligner.get_keypoints(args.img1)
#            kptsB_orig = aligner.get_keypoints(args.img2)
            # Load & reconcile keypoints by COCO index (fill missing by similarity-imputation)
            kptsA_orig, kptsB_orig = None, None
            try:
                kptsA_orig, kptsB_orig = reconcile_keypoints_by_index(args.keypoints_json, args.img1, args.img2)
                print(f"Loaded keypoints (original space): A={len(kptsA_orig)}, B={len(kptsB_orig)} (after reconciliation)")
            except Exception as e:
                print(f"WARNING: reconcile_keypoints_by_index failed ({e}). Falling back to basic loader.")
                aligner = KeypointAligner(args.keypoints_json)
                kA_tmp = aligner.get_keypoints(args.img1)
                kB_tmp = aligner.get_keypoints(args.img2)
                if kA_tmp is not None and kA_tmp.shape[1] == 3:
                    kA_tmp = kA_tmp[:, :2]
                if kB_tmp is not None and kB_tmp.shape[1] == 3:
                    kB_tmp = kB_tmp[:, :2]
                kptsA_orig, kptsB_orig = kA_tmp, kB_tmp

            if kptsA_orig is None or kptsB_orig is None:
                print("ERROR: No keypoints found for one or both images. Skipping TPS homology for this pair.")
                kptsA = kptsB = None
            else:
                ...
                def scale_kpts_to_preserved_aspect(kpts, img_orig, img_size):
                    ...
                    k2 = kpts * scale
                    k2[:, 0] += (img_size - new_w) / 2
                    k2[:, 1] += (img_size - new_h) / 2
                    return k2

                kptsA_scaled = scale_kpts_to_preserved_aspect(kptsA_orig.copy(), imgA_raw, img_size)
                kptsB_scaled = scale_kpts_to_preserved_aspect(kptsB_orig.copy(), imgB_raw, img_size)

                # Apply crop transforms (if any)
                if not args.no_crop and crop_bbox_A is not None:
                    x0, y0, x1, y1 = crop_bbox_A
                    crop_w, crop_h = x1 - x0, y1 - y0
                    k = kptsA_scaled
                    k[:, 0] -= x0;  k[:, 1] -= y0
                    sc = min(img_size / crop_w, img_size / crop_h)
                    new_w = int(crop_w * sc); new_h = int(crop_h * sc)
                    k *= sc
                    k[:, 0] += (img_size - new_w) / 2
                    k[:, 1] += (img_size - new_h) / 2
                    kptsA_scaled = k
                ...
                # Apply affine (if images were aligned)
                if affine is not None:
                    ones = np.ones((len(kptsA_scaled), 1))
                    kA_h = np.hstack([kptsA_scaled, ones])
                    kptsA_scaled = (affine @ kA_h.T).T
                ...
                # Constrain to canvas & masks, but DO NOT drop indices
                kptsA, _ = clip_keypoints_to_canvas(kptsA, img_size, "Image A")
                kptsB, _ = clip_keypoints_to_canvas(kptsB, img_size, "Image B")

                print("Constraining keypoints to foreground mask regions...")
                kptsA, _ = constrain_keypoints_to_mask(kptsA, maskA, img_size, safety_margin=5, identifier="Image A")
                kptsB, _ = constrain_keypoints_to_mask(kptsB, maskB, img_size, safety_margin=5, identifier="Image B")

                # If somehow lengths differ, pad by NaNs and keep indices aligned (should not happen)
                if kptsA.shape[0] != kptsB.shape[0]:
                    N = max(kptsA.shape[0], kptsB.shape[0])
                    kA2 = np.full((N, 2), np.nan, float)
                    kB2 = np.full((N, 2), np.nan, float)
                    kA2[:kptsA.shape[0]] = kptsA
                    kB2[:kptsB.shape[0]] = kptsB
                    kptsA, kptsB = kA2, kB2

                # Count mismatch is handled by reconciliation; do NOT skip TPS.
                tps_homology_skipped = False
                 # Load or compute consensus shape
                consensus_shape = None
                if not tps_homology_skipped:
                    ...
                    print("Skipping consensus shape because TPS homology is disabled for this pair.")
                ...
                # End of: if kptsA is not None and kptsB is not None
                # (keypoints were valid and counts matched)
